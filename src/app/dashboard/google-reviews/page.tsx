...
function Toggle({ on }: { on: boolean }) {
  return (
    <div className={`flex h-4 w-7 items-center rounded-full px-0.5 ${on ? "bg-coral" : "bg-gray-300"}`}>
...
export default function GoogleReviewsPage() {
  ...
  var metrics = [
    { label: "Overall rating", value: avgRating + " \u2605", change: totalCount + " total", accent: "border-t-amber-400", pill: "bg-emerald-accent/10 text-coral-deep" },
    { label: "Total reviews", value: String(totalCount), change: "from all sources", accent: "border-t-blue-500", pill: "bg-gray-100 text-text-dim" },
    { label: "Pending replies", value: String(pendingCount), change: pendingCount > 0 ? "needs attention" : "all caught up", accent: "border-t-purple-500", pill: pendingCount > 0 ? "bg-red-50 text-red-600" : "bg-emerald-accent/10 text-coral-deep" },
    { label: "AI replied", value: String(repliedCount), change: totalCount > 0 ? Math.round((repliedCount / totalCount) * 100) + "% reply rate" : "—", accent: "border-t-emerald-500", pill: "bg-emerald-accent/10 text-coral-deep" },
    { label: "Sentiment", value: sentimentPct + "%", change: "Positive (\u22654\u2605)", accent: "border-t-pink-500", pill: "bg-emerald-accent/10 text-coral-deep" },
  ...
  var sentimentColors = ["bg-red-500", "bg-red-400", "bg-amber-400", "bg-emerald-400", "bg-coral"];
  ...
  var avatarColors = ["bg-blue-500", "bg-purple-500", "bg-pink-500", "bg-amber-500", "bg-coral", "bg-red-500", "bg-cyan-500"];
  ...
  function getStatusBadge(status: string, rating: number) {
    if (status === "approved" || status === "replied") {
      return { text: "Replied", cls: "border-emerald-100 bg-emerald-accent/10 text-coral-deep" };
    }
    if (rating <= 2) {
  ...
  return (
    <div className="px-8 py-6">
      ...
      <div className="mb-5 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-ink">Reviews AI</h1>
          <p className="mt-1 text-sm text-text-muted">Your review-powered marketing engine &mdash; every review becomes social proof, content, and patients</p>
        </div>
        <div className="flex gap-2">
          <button className="cursor-pointer rounded-xl border border-gray-200 px-4 py-2 text-xs font-medium text-ink shadow-sm transition-all duration-200 hover:border-coral hover:text-coral-deep">Request reviews</button>
          <button className="cursor-pointer rounded-xl bg-coral px-4 py-2 text-xs font-medium text-white shadow-sm transition-all duration-200 hover:bg-coral-deep hover:shadow-md">
      ...
      <div className="mb-5 grid grid-cols-6 gap-3">
        {metrics.map((m) => (
          <div key={m.label} className={`rounded-2xl border border-gray-100 border-t-2 bg-white p-4 shadow-sm transition-all duration-200 hover:shadow-md ${m.accent}`}>
            <div className="text-[10px] font-medium uppercase tracking-wider text-text-muted">{m.label}</div>
            <div className="mt-1 text-2xl font-semibold text-ink">{m.value}</div>
          ...
          return (
            <button
              ...
              className={`cursor-pointer pb-2 pr-5 text-sm transition-all duration-200 ${
                filter === t.id ? "border-b-2 border-coral font-medium text-ink" : "text-text-muted hover:text-text-dim"
              }`}
            >
              {t.label}
              <span className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[9px] font-medium ${
                t.id === "pending" && count > 0 ? "bg-red-50 text-red-600" : "bg-gray-100 text-text-muted"
              }`}>{count}</span>
            </button>
          );
        })}
      ...
      <div className="grid grid-cols-[1fr_280px] gap-5">
        ...
        <div>
          {loading ? (
            <div className="flex min-h-[30vh] items-center justify-center">
              <div className="text-sm text-text-muted">Loading reviews...</div>
            </div>
                ...
                >
                  ...
                  <div className="mb-3 flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="text-[13px] font-medium text-ink">{review.reviewer_name}</div>
                      <div className="mt-0.5 flex items-center gap-2">
                        <span className="text-[11px] text-text-muted">{review.time || review.created_at || ""}</span>
                  ...
                  {/* Review text */}
                  <div className="text-[13px] leading-relaxed text-ink">{review.review_text}</div>

                  {/* AI reply section — pending reviews */}
                  {isPending && (
                    <div className={`mt-3 rounded-xl border p-4 ${isNegative ? "border-red-100/50 bg-red-50/30" : "border-emerald-100/50 bg-emerald-accent/10/30"}`}>
                      <div className="mb-2 flex items-center gap-2">
                        <span className={`rounded-md px-2.5 py-0.5 text-[9px] font-medium text-white ${isNegative ? "bg-red-500" : "bg-coral"}`}>
                      ...
                      {replyText && (
                        <div className="text-[12px] italic leading-relaxed text-ink">{replyText}</div>
                      )}
                      <div className="mt-3 flex gap-2">
                        <button
                          onClick={() => handleApprove(review)}
                          disabled={approving[review.id] || !replyText}
                          className="cursor-pointer rounded-lg bg-coral px-3 py-1.5 text-[10px] font-medium text-white shadow-sm transition-all duration-200 hover:bg-coral-deep disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          {approving[review.id] ? "Posting..." : "Approve + post"}
                        </button>
                        <button className="cursor-pointer rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-[10px] text-ink transition-all duration-200 hover:border-coral">Edit reply</button>
                        <button
                          onClick={() => handleRegenerate(review)}
                          disabled={regenerating[review.id]}
                          className="cursor-pointer rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-[10px] text-ink transition-all duration-200 hover:border-coral disabled:cursor-not-allowed disabled:opacity-50"
                        >
                  ...
                  {isReplied && review.ai_reply && (
                    <div className="mt-2 text-[10px] italic text-coral">
        ...
        <div>
          ...
          <div className="mb-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <div className="text-sm font-medium text-ink">&#10022; Auto-marketing rules</div>
            <div className="mb-3 text-[10px] text-text-muted">Every review triggers these automatically</div>
            {[
              "Review \u2192 website feed",
              "Review \u2192 social post",
              "Review \u2192 GBP update",
          ...
          <div className="mb-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <div className="mb-2 text-sm font-medium text-ink">Request campaigns</div>
            {[
              { icon: "W", color: "bg-coral", name: "WhatsApp", when: "2hr after visit", on: true },
              ...
              <div key={c.name} className="flex items-center gap-2 border-b border-gray-50 py-2">
                ...
                <span className="text-[9px] text-text-muted">{c.when}</span>
                <Toggle on={c.on} />
              </div>
            ))}
            <div className="border-t border-gray-100 pt-2 text-[10px] text-text-muted">Smart gating: Happy &rarr; Google review. Unhappy &rarr; private feedback &rarr; recovery.</div>
          </div>

          {/* Card 3: Competitor watch */}
          <div className="mb-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <div className="mb-2 text-sm font-medium text-ink">Competitor watch</div>
            {[
              ...
              <div key={c.name} className="flex items-center justify-between border-b border-gray-50 py-2">
                ...
                <span className="text-[11px] text-text-muted">{c.rating}</span>
              </div>
            ))}
            <div className="mt-2 rounded-lg border border-emerald-100 bg-emerald-accent/10 p-2 text-[10px] text-coral-deep">
          ...
          <div className="mb-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <div className="mb-2 text-sm font-medium text-ink">Sentiment breakdown</div>
            {sentimentBars.map((s) => (
              <div key={s.stars} className="flex items-center gap-2 py-1">
                <span className="w-4 text-right text-[10px] text-text-muted">{s.stars}&#9733;</span>
                <div className="h-1.5 flex-1 rounded-full bg-gray-100">
                  <div className={`h-full rounded-full ${s.color}`} style={{ width: `${s.pct}%` }} />
                </div>
                <span className="w-12 text-right text-[10px] text-text-muted">{s.count} ({s.pct}%)</span>
              </div>
            ))}
          </div>
          ...
          <div className="mb-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <div className="mb-2 text-sm font-medium text-ink">Platform monitoring</div>
            {[
              { icon: "G", color: "bg-blue-500", name: "Google", rating: avgRating + " \u00b7 " + totalCount + " reviews", badge: "Connected", badgeClass: "bg-emerald-accent/10 text-coral-deep" },
              { icon: "P", color: "bg-pink-500", name: "Practo", rating: "Monitoring", badge: "Monitoring", badgeClass: "bg-amber-50 text-amber-600" },
              { icon: "f", color: "bg-blue-600", name: "Facebook", rating: "Connected", badge: "Connected", badgeClass: "bg-emerald-accent/10 text-coral-deep" },
            ].map((p) => (
              <div key={p.name} className="flex items-center gap-2 border-b border-gray-50 py-2">
                ...
                <span className="flex-1 text-[11px] text-ink">{p.name}</span>
