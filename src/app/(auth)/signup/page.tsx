...
function validate(f: { business_name: string; email: string; phone: string; password: string }) {
  var e: Record<string, string> = {};
  if (!f.business_name || f.business_name.length < 2) e.business_name = "Clinic name is required";
  if (!f.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.email)) e.email = "Valid email required";
  if (!f.phone || f.phone.replace(/[\s-]/g, "").length < 10) e.phone = "Valid phone required";
...
export default function SignupPage() {
  ...
  function onChange(field: string, value: string) {
    ...
    if (touched[field]) {
      var v = validate({ ...form, [field]: value });
      setErrors((prev) => {
        var next = { ...prev };
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setApiError("");
    var res = await signupUser(form.email, form.business_name, form.phone.replace(/[\s-]/g, ""), form.password);
    setLoading(false);
  ...
  return (
    <div className="w-full max-w-[380px]">
      ...
      <p className="mb-6 text-[12px] leading-relaxed text-gray-400">
        AI runs your clinic&apos;s marketing across every channel. Website, social, reviews, WhatsApp — all on autopilot.
      </p>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        {/* Clinic name */}
        <div>
          <label className="mb-1 block text-[11px] uppercase tracking-wider text-gray-500">Clinic name</label>
          <input
        ...
        <div>
          ...
          <div className="flex gap-2">
            <div className="flex h-[42px] w-[60px] items-center justify-center rounded-lg border border-[#1f2e28] bg-[#111916] text-[12px] text-gray-400">
              +91
            </div>
            <input
        ...
        >
          {loading ? (
            ...
            <>Start free <span className="text-[13px]">→</span></>
          )}
        </button>
      </form>
      ...
      <div className="mt-4 flex items-center justify-center gap-4">
        {["No card needed", "HIPAA ready", "Cancel anytime"].map((t) => (
